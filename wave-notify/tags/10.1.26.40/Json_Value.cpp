...
//      wchar_t *newString = static_cast<wchar_t *>( malloc( length * sizeof(wchar_t) ) );
//      memcpy( newString, czstring, length * sizeof(wchar_t) );
//      return newString;
//   }
//   return 0;
//}
...
//      memcpy( newString, str.c_str(), length * sizeof(wchar_t) );
//      newString[length] = 0;
//      return newString;
//   }
//   return 0;
...
Value::CommentInfo::setComment( const wchar_t *text )
{
   if ( comment_ )
      valueAllocator()->releaseStringValue( comment_ );
   ASSERT( text );
   ASSERT_MSG( text[0]==L'\0' || text[0]==L'/', "Comments must start with /");
...
Value::asInt() const
{
   ...
   case intValue:
      return value_.int_;
   case uintValue:
      ASSERT_MSG( value_.uint_ < (unsigned)maxInt, "integer out of signed integer range" );
      return value_.uint_;
   case realValue:
      ASSERT_MSG( value_.real_ >= minInt  &&  value_.real_ <= maxInt, "Real out of signed integer range" );
   ...
   case arrayValue:
   case objectValue:
      ASSERT_MSG( false, "Type is not convertible to int" );
   default:
      ASSERT(FALSE);
   }
   return 0; // unreachable;
...
Value::asUInt() const
{
   ...
   case nullValue:
      return 0;
   case intValue:
      ASSERT_MSG( value_.int_ >= 0, "Negative integer can not be converted to unsigned integer" );
      return value_.int_;
   case uintValue:
      return value_.uint_;
   case realValue:
      ASSERT_MSG( value_.real_ >= 0  &&  value_.real_ <= maxUInt,  "Real out of unsigned integer range" );
   ...
   case arrayValue:
   case objectValue:
      ASSERT_MSG( false, "Type is not convertible to uint" );
   default:
      ASSERT(FALSE);
   }
   return 0; // unreachable;
...
Value::asDouble() const
{
   ...
   case arrayValue:
   case objectValue:
      ASSERT_MSG( false, "Type is not convertible to double" );
   default:
      ASSERT(FALSE);
   }
   return 0; // unreachable;
      ...
      if ( *current == L'[' )
      {
         ...
         else
         {
            ...
            for ( ; current != end && *current >= L'0'  &&  *current <= L'9'; ++current )
               index = index * 10 + Value::UInt(*current - L'0');
